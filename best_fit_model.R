...
library(leaps)
#exhaustive search
regfit.best <- regsubsets(average_wait_time ~ .,
                          data = training_set,
                          nvmax = 22) 
...
best.mod <- lm(average_wait_time~arrival_rate+queue_length+avg_items_per_customer
...
predicted.best <- predict(best.mod,validation_set)

forecast::accuracy(predicted.best,validation_set$average_wait_time)
